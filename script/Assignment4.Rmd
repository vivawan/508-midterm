...
stargazer(mod1, type = "text",  star.cutoffs = c(0.05, 0.01, 0.001))

### Remove Education_Level
mod2 <- glm(Recidivism_Within_3years ~ . - Education_Level,
                 data = dat.train,
                 na.action = na.exclude,
                 family = binomial("logit"))
stargazer(mod2, type = "text",  star.cutoffs = c(0.05, 0.01, 0.001))

### Remove Condition_Cog_Ed
mod3 <- glm(Recidivism_Within_3years ~ . - Education_Level - Condition_Cog_Ed,
                 data = dat.train,
                 na.action = na.exclude,
                 family = binomial("logit"))
stargazer(mod3, type = "text",  star.cutoffs = c(0.05, 0.01, 0.001))

### Remove Condition_Other
mod4 <- glm(Recidivism_Within_3years ~ . - Education_Level - Condition_Cog_Ed - Condition_Other,
                 data = dat.train,
                 na.action = na.exclude,
                 family = binomial("logit"))
stargazer(mod4, type = "text",  star.cutoffs = c(0.05, 0.01, 0.001))

### Remove Employment_Exempt
mod5 <- glm(Recidivism_Within_3years ~ . - Education_Level - Condition_Cog_Ed - Condition_Other
            - Employment_Exempt,
                 data = dat.train,
                 na.action = na.exclude,
                 family = binomial("logit"))
...
mod6 <- glm(Recidivism_Within_3years ~ . - Education_Level - Condition_Cog_Ed - Condition_Other
            - Employment_Exempt - Delinquency_Reports,
                 data = dat.train,
                 na.action = na.exclude,
                 family = binomial("logit"))
stargazer(mod6, type = "text",  star.cutoffs = c(0.05, 0.01, 0.001))

### Remove Program_Attendances
mod7 <- glm(Recidivism_Within_3years ~ . - Education_Level - Condition_Cog_Ed - Condition_Other
            - Employment_Exempt - Delinquency_Reports - Program_Attendances,
                 data = dat.train,
                 na.action = na.exclude,
                 family = binomial("logit"))
stargazer(mod7, type = "text",  star.cutoffs = c(0.05, 0.01, 0.001))

### Remove Program_UnexcusedAbsences
mod8 <- glm(Recidivism_Within_3years ~ . - Education_Level - Condition_Cog_Ed - Condition_Other
            - Employment_Exempt - Delinquency_Reports - Program_Attendances - Program_UnexcusedAbsences,
                 data = dat.train,
                 na.action = na.exclude,
                 family = binomial("logit"))
stargazer(mod8, type = "text",  star.cutoffs = c(0.05, 0.01, 0.001))

### Remove Percent_Days_Employed
mod9 <- glm(Recidivism_Within_3years ~ . - Education_Level - Condition_Cog_Ed - Condition_Other
            - Employment_Exempt - Delinquency_Reports - Program_Attendances - Program_UnexcusedAbsences
            - Supervision_Risk_Score_First - Percent_Days_Employed,
                 data = dat.train,
                 na.action = na.exclude,
                 family = binomial("logit"))
stargazer(mod9, type = "text",  star.cutoffs = c(0.05, 0.01, 0.001))

### Remove Prison_Years
mod10 <- glm(Recidivism_Within_3years ~ . - Education_Level - Condition_Cog_Ed - Condition_Other
            - Employment_Exempt - Delinquency_Reports - Program_Attendances - Program_UnexcusedAbsences
            - Supervision_Risk_Score_First - Percent_Days_Employed - Prison_Years,
                 data = dat.train,
                 na.action = na.exclude,
                 family = binomial("logit"))
stargazer(mod10, type = "text",  star.cutoffs = c(0.05, 0.01, 0.001))

### Remove Prior_Arrest_Episodes_Misd
mod11 <- glm(Recidivism_Within_3years ~ . - Education_Level - Condition_Cog_Ed - Condition_Other
            - Employment_Exempt - Delinquency_Reports - Program_Attendances - Program_UnexcusedAbsences
            - Supervision_Risk_Score_First - Percent_Days_Employed - Prison_Years - Prior_Conviction_Episodes_Misd,
                 data = dat.train,
                 na.action = na.exclude,
                 family = binomial("logit"))
stargazer(mod11, type = "text",  star.cutoffs = c(0.05, 0.01, 0.001))

### Model performance
testProbs <- data.frame(Outcome = as.factor(dat.test$Recidivism_Within_3years),
                        Probs = predict(mod13, dat.test, type= "response")) %>% ### Test probabilities
  mutate(predOutcome  = as.factor(ifelse(Probs > 0.5 , 1, 0))) ### Set thresholds

caret::confusionMatrix(testProbs$predOutcome, testProbs$Outcome, 
                       positive = "1") ### Confusion matrix

pROC::auc(testProbs$Outcome, testProbs$Probs)
### If sensitivity goes up and specificity goes down, the variable is removed. (minimize False Negative)
```
