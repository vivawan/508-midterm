...
house <- hh %>% 
  mutate(year_used = 2015 - yr_built, # used year
         reno_dum = if_else(yr_renovated>0, 1, 0),#renovation yes or no
...
house %>%
  ...
  plotTheme() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) #0-3,4-9,11+
house <- house %>%
  mutate(bed_cat = factor(case_when(
    bedrooms <=3 ~ "few",
    bedrooms >3 | bedrooms <= 9 ~ "medium",
    bedrooms >9 ~ "many"
  )))%>%
  select(-bed.factor)
...
house %>%
  ...
  geom_col(position = "dodge")+
  plotTheme() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) #0, 0.5-4, 4+
house <- house %>%
  mutate(bath_cat = factor(case_when(
    bathrooms == 0 ~ "no",
    bathrooms >0 | bedrooms <= 4 ~ "few",
    bathrooms > 4 ~ "many"
  )))%>%
  select(-bath.factor)
...
house <- house %>%
  select("key", "price","year_used","reno_dum",
         "bedrooms", "bed_cat", "bathrooms", "bath_cat",
         "sqft_living", "sqft_lot", "floors", "floor_cat",
         "water_dum", "view_cat", "condition_cat", "grade_cat")
```
...
acsTractsSeattle.2015 <- get_acs(geography = "tract",
  ...
  mutate(pop_den = ifelse(as.numeric(area) > 0, total_pop / area, 0),
         ...
         bach_cat = ifelse(bach_share > mean(acsTractsSeattle.2015$bach_share,
                                                        na.rm = TRUE), "above", "below"),
         median_hh_cat = ifelse(median_hh_income > mean(acsTractsSeattle.2015$median_hh_income,
                                                        na.rm = TRUE), "above", "below"),
         employ_rate = round(ifelse(total_labor > 0, employ_labor / total_labor, 0) * 100, digits = 2),
         employ_cat = ifelse(employ_rate > mean(acsTractsSeattle.2015$employ_rate,
                                                        na.rm = TRUE), "above", "below"),
         pover_rate = round(ifelse(total_pop > 0, poverty / total_pop, 0) * 100, digits = 2),
         pover_cat = ifelse(pover_rate > mean(acsTractsSeattle.2015$pover_rate,
                                                        na.rm = TRUE), "above", "below")) %>%
  select(GEOID, NAME, pop_den, white_share, total_dep, elder_dep, bach_share, bach_cat, median_hh_income,
         median_hh_cat, employ_rate, employ_cat, pover_rate, pover_cat)
...
house <- house %>% 
    left_join(house_parks%>%
                st_drop_geometry()%>%
                select(key,park_c, sum_park_area), by = "key")%>%
  rename(parks_area = "sum_park_area")
```
