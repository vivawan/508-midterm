...
continuous_description <- data.frame(
  ...
  description = c("price of each unit",
                  ...
                  "the area of the lot of each unit",
...
house %>%
  ...
  summarise(count = n()) %>%
  mutate(percent = round(count/sum(count) * 100, digits = 2))%>%
  mutate(bed_cat = factor(bath_dum, levels = levels(house$bath_dum))) %>%
  arrange(bath_dum)%>%
...
house %>%
  ...
  summarise(count = n()) %>%
  mutate(percent = round(count/sum(count) * 100, digits = 2))%>%
  mutate(bed_cat = factor(floor_cat, levels = levels(house$floor_cat))) %>%
  arrange(floor_cat)%>%
  mutate(description = c("the unit has 1/1.5/2/3 floors", 
                         "the unit has 2.5/3.5 floors"))%>%
  kable() %>%
  kable_styling()
...
house %>%
  ...
  mutate(description = c("the unit is within a 0.5-mile radius of a subway station", 
                         "the unit is without a 1-mile radius of a subway station",
                         "the unit is between a 0.5-mile to 1-mile radius of a subway station"))%>%
  kable() %>%
  kable_styling()
...
house %>%
  ...
  mutate(bed_cat = factor(floor_cat, levels = levels(house$floor_cat))) %>%
  arrange(floor_cat)%>%
  mutate(description = c("the unit is in school district DD7",
                         "the unit is in school district DD1",
                         "the unit is in school district DD6",
                         "the unit is in school district DD3",
                         "the unit is in school district DD2",
                         "the unit is in school district DD4",
                         "the unit is in school district DD5"))%>%
  kable() %>%
  kable_styling()
...
house %>%
  ...
  mutate(bed_cat = factor(floor_cat, levels = levels(house$floor_cat))) %>%
  arrange(floor_cat)%>%
  mutate(description = c("the unit is without a 1-mile radius of a shopping center",
                         "the unit is between a 0.5-mile to 1-mile radius of a shopping center",
                         "the unit is within a 0.5-mile radius of a shopping center"))%>%
  kable() %>%
  kable_styling()
...
ggplot() +
  ...
  geom_sf(data = st_as_sf(house%>%left_join(.,hh%>%select(key, geometry), by = "key")),
...
ggplot() +
  geom_sf(data = seattle, fill = "#ECECEC", color = "#2166ac", linewidth = 0.5) +
  geom_sf(data = st_as_sf(house%>%left_join(.,hh%>%select(key, geometry), by = "key")),
          aes(color = cut(sqft_lot, breaks = breaks_quantiles$brks, include.lowest = TRUE)), size = 1) +
  scale_color_manual(values = colors,
                    labels = labels,
