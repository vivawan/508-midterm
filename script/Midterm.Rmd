...
```{r read_show,cache=TRUE, eval=FALSE}
## read Seattle boundary
seattle <- st_read(here::here("data/raw/Boundary/Seattle_City.geojson")) %>%
  st_union() %>%
  st_as_sf() 
...
```{r internal, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r socio-economic variable, results='hide'}
...
```{r subway, echo=TRUE, message=FALSE, warning=FALSE}
sub <- sub %>%
...
```{r school, echo=TRUE, message=FALSE, warning=FALSE}
sch <- sch %>%
  st_transform(st_crs(house))
...
```{r park, echo=TRUE, message=FALSE, warning=FALSE}
park <- park %>%
  st_transform(st_crs(house))%>%
  st_intersection(seattle)%>%
...
```{r tree_canopy, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r medical, echo=TRUE, message=FALSE, warning=FALSE}
med <- med %>%
  st_transform(st_crs(house))%>%
  st_intersection(seattle)
...
```{r commercial, echo=TRUE, message=FALSE, warning=FALSE}
mark <- mark %>%
  st_transform(st_crs(house))%>%
  st_intersection(seattle)
...
```{r crime_clean, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r continuous_clean, echo=TRUE, message=FALSE, warning=FALSE}
# exclude outlier
house <- house%>%
...
```{r category mean plot, echo=TRUE, message=FALSE, warning=FALSE}
