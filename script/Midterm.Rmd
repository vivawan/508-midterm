...
seattle.test.nhood <-
  ...
  mutate(regression = "neighborhood effects", # add a column indicating the type of regression model used
         price.predict = predict(lm.final.nhood, seattle.test.lm), # predict house prices using the trained regression model
         # calculate the difference between predicted and actual house prices
         price.error = price.predict - price,
         # calculate the absolute difference between predicted and actual house prices
         price.AbsError = abs(price.predict - price),
...
mean(seattle.test$price.AbsError) # 93073.76
mean(seattle.test7$price.APE) # 18.57%
...
bothRegressions <- 
  rbind(
    select(boston.test, starts_with("SalePrice"), Regression, Name) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)),
    select(boston.test.nhood, starts_with("SalePrice"), Regression, Name) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)))  
```
