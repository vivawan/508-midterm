...
house <- house %>%
  mutate(parks_area_log = log(parks_area))
  select(-GEOID, -NAME, -key) %>%
  st_drop_geometry()
str(house)

lm1 <- lm(price ~ ., data = house)
summary(lm1)
...
lm1 <- lm(price ~ ., data = seattle.train)
summary(lm1)
vif(lm1)

# shop_dis
lm2 <- lm(price ~ .-shop_dis2-shop_dis3, data = seattle.train)
summary(lm2)
vif(lm2)
...
# med_dis
lm3 <- lm(price ~ .-med_dis1-med_dis2, data = seattle.train)
summary(lm3)
vif(lm3)

seattle.train <- seattle.train %>%
  select(-med_dis1, -med_dis2)

# crime_dis
lm4 <- lm(price ~ .-crime_dis2-crime_dis3-crime_dis4-crime_dis5, data = seattle.train)
summary(lm4)
vif(lm4)

seattle.train <- seattle.train %>%
  select(-crime_dis2, -crime_dis3, -crime_dis4, -crime_dis5)
cor(seattle.train%>%select(year_used, bedrooms, bathrooms, sqft_living,
                               sqft_lot, pop_den, white_share, total_dep,
                               elder_dep, bach_share, median_hh_income, employ_rate,
                               pover_rate, sub_dis, parks_area, tree_canopy, med_dis3,
                               shop_dis1, crime_c, crime_dis1))

# sub_dis & sub_cat
lm5 <- lm(price ~ .-sub_cat, data = seattle.train)
summary(lm5)
vif(lm5)
...
# crime_c & crime_dis
lm6 <- lm(price ~ .-crime_dis1, data = seattle.train)
summary(lm6)
vif(lm6)
...
# shop_dis & shop_cat
lm7 <- lm(price ~ .-shop_cat, data = seattle.train)
summary(lm7)
vif(lm7)
...
# parks_area & park_cat
lm8 <- lm(price ~ .-parks_area, data = seattle.train)
summary(lm8)
vif(lm8)
